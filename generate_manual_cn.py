...
h2('3.4  序列号（SN）')
para('每个文件夹都可以设置可选的序列号（SN），用于标记组件、批次或项目编号。')
bullet('单击文件夹上的铅笔图标，打开编辑模式。')
bullet('在序列号字段中输入值（例如 INS-2026-001）。')
bullet('SN 将显示在树形视图中该文件夹名称旁边。')
figure('02_folder_with_sn.png', '图 3-1　带序列号的文件夹（房屋保险 · INS-2026-001）')
figure('03_folder_edit_mode.png', '图 3-2　文件夹编辑模式：名称、序列号与登录信息字段')

h2('3.5  描述：语音输入、AI 整理与导出 PDF')
para('每个文件夹（以及每份文档）都有一块独立的"描述"区域——这不只是一行备注，而是一整套完整的写作与整理工具：语音输入、上传录音自动转文字、AI 整理润色、错别字检查，最后还能一键存为可持续更新的 PDF。文件夹层级的描述框最早只是一个简单的文本框，现已升级为与单个文档完全相同的完整功能面板。')
bullet('【语音输入】——单击后对着麦克风说话，识别结果会自动追加到描述末尾；可在"中文／EN"之间切换识别语言。识别在浏览器中完成，无需配置 AI。')
bullet('【上传录音转文字】——选择一段录音文件（如手机上的语音备忘录 .m4a，也支持 .mp3/.wav/.aac 等格式），由 AI（Gemini）转写为文字并追加到描述中——是"语音输入"之外的另一种录入方式，尤其适合先在手机上录好、之后再统一处理的场景（见下方"场景二"）。首次使用前需配置 Gemini API Key，见下方说明。')
bullet('【AI 整理】——从"整理结构 / 润色文字 / 精简概括"三种方式中选择，单击【用 AI 整理润色】，AI 会在保留全部信息的前提下把口述或随手写的内容整理为通顺的文字；结果会先展示预览，单击【应用】才会替换原文，不会静默覆盖。')
bullet('【检查错别字】——用 AI 检查描述中的错别字、标点与成语误用，可逐条应用修正，或一键【全部应用】。')
bullet('【保存并可搜索】——把当前描述存入索引，之后可通过关键词搜索到。')
bullet('【另存为新文档】／【更新描述文档】——把描述另存为同一文件夹中的一份 .txt 文档；再次保存时按钮会变为"更新描述文档"，覆盖同一份文件，不会越存越多。')
bullet('【保存为 PDF】／【更新 PDF】——把描述生成一份格式规范的 PDF 文档，同样存入所在文件夹；描述内容修改后再次点击会自动更新同一份 PDF（文件名不变），适合作为可以随时打印或分享的正式记录。')
figure('23_folder_description_panel.png', '图 3-3　文件夹的描述面板（示例："宝宝出生"文件夹）')
tip('场景一・成长纪念册：新建一个"宝宝出生"文件夹，在描述里完整记录这段故事——几点赶去的医院、医院和医生的名字、第一次抱到宝宝时的心情……同时把出生证、小脚印、B 超单等资料作为文档一起放进这个文件夹。有了这段整体描述，文件夹里零散的证件和照片就串成了一本完整的成长纪念册。')
tip('场景二・语音记录日常测量：每次量完血压、血糖后，直接对手机说出结果（或用备忘录 App 录一段语音），多次测量都先存在手机上；方便时再把这些录音传到电脑（如通过 AirDrop、微信文件传输、数据线等）。打开对应文件夹或文档的描述面板，逐个点击【上传录音转文字】，录音会依次转写并追加到描述里；最后点一次【用 AI 整理润色】（选择"整理结构"），零散的多次口述就会被自动梳理成一份规范的测量记录，再点击【保存为 PDF】留存即可。')
note('上传录音转文字、AI 整理、检查错别字这三项都需要先配置 Gemini API Key：未配置时，对应按钮位置会显示一个"配置 AI（Gemini）"链接，单击后按提示粘贴您的 Key 并保存即可（该 Key 只保存在本机，加密存储）。语音输入（麦克风实时识别）不需要配置任何 Key，可直接使用。')
...
figure('04_login_info_view.png', '图 3-4　已保存登录信息后的查看模式')
...
numbered('查看器顶部有【添加描述】【查看地图位置】【提取文字（OCR）】【提取关键参数】【扫描条码】（见第 4.8 节）【提问 AI】（需先配置 API Key，见第 3.5 节）【下载】【关闭】等操作按钮。')
...
h2('4.8  扫描条码（识别图片中的条码/二维码）')
para('如果一张照片里拍到了条码或二维码（例如产品标签、设备铭牌），DMS 可以直接从图片中把编码的内容识别出来，无需手动抄录。')
numbered('打开该图片（或含条码图片的 PDF）的查看器。')
numbered('单击顶部的【扫描条码】按钮。')
numbered('识别成功后，顶部会出现一条提示，显示识别到的编码类型与内容（例如"CODE128：6901234567892"）。')
numbered('识别结果会自动存为该文档的"Barcode"参数，可在下方"元数据"区域查看或编辑；同一张照片上有多个条码时，会依次存为 Barcode、Barcode2、Barcode3……')
figure('24_scan_barcode.png', '图 4-3　扫描条码：识别成功后的提示')
tip('照片的拍摄日期和 GPS 位置无需额外操作——上传时 DMS 已自动从照片的 EXIF 信息中提取并保存（见第 4.1 节），"扫描条码"只补上条码本身的编码内容。三者合在一起，一张照片就能同时留下"什么时候、在哪里、编码是什么"三项记录。')
note('本功能完全离线运行（基于开源的 zbar 库），不上传任何数据，也不需要配置 AI Key，但对图片质量有一定要求：条码需清晰、无遮挡，且条码四周需留有足够的空白边框（"静区"）。手机截图分享出来的二维码（例如微信"扫码入群"图片）常常因为中间嵌有较大的图标、或背景色直接顶到条码边缘而无法识别——这类图片通常只有微信自己的扫一扫或手机相机能读取，属于本功能的已知局限，并非软件故障。')
pb()
...
# 14 密码
h1('14  密码保护')
para('您可以为 DMS 设置密码，确保只有授权用户才能访问数据，尤其是在启用了局域网访问、远程上传或保存了网站登录信息（第 3.6 节）之后，强烈建议设置密码。')
para('设置密码：')
numbered('单击【项目】→【设置密码】。')
...
note('密码以加盐哈希方式存储在服务器端（SHA-256，100,000 次迭代），不以明文保存。文件夹的网站登录密码（第 3.6 节）则使用另一套独立的可逆加密机制存储，二者互不影响。')
pb()
...
table(['问题','解决方法'],[
    ...
    ['扫描条码提示"未在图片中识别到条码"',
     '请确认条码本身清晰、无遮挡，且四周留有空白边框。手机截图分享的二维码（尤其是中间嵌有图标、或背景色直接顶到条码边缘的，如微信"扫码入群"图片）通常无法被本功能识别，只有微信自己的扫一扫或手机相机能读取，见第 4.8 节。'],
    ['"上传录音转文字"或"用 AI 整理润色"按钮位置显示"配置 AI（Gemini）"',
     '说明尚未配置 Gemini API Key。单击该链接，按提示粘贴您的 Key 并保存即可，见第 3.5 节。'],
    ['转写录音后描述内容没有变化，或提示转写失败',
     '请确认录音中确实包含清晰的人声（纯音乐、静音或环境噪音会被判定为"未检测到语音内容"）；若提示"UNAVAILABLE"或"高需求"，通常是 Gemini 服务临时繁忙，稍后重试即可。'],
],[2.5,4.0])
doc.add_paragraph()
h2('获取帮助')
...
table(['任务','操作方法'],[
    ...
    ['设置访问密码','项目菜单 → 设置密码'],
    ['配置邮件发送','项目菜单 → 邮件发送设置'],
    ['为文件夹/文档语音录入描述','描述面板 → 语音输入 → 选择中文/EN → 对着麦克风说话'],
    ['上传录音（如手机备忘录）转为描述文字','描述面板 → 上传录音转文字 → 选择录音文件'],
    ['用 AI 整理/润色描述','描述面板 → 选择整理方式 → 用 AI 整理润色 → 应用'],
    ['把描述保存为可更新的 PDF','描述面板 → 保存为 PDF（再次编辑后点击同一按钮即可更新同一份文件）'],
    ['识别照片中的条码/二维码','打开图片查看器 → 扫描条码'],
],[2.5,4.5])
