...
class LocalFolderStructureTests(unittest.TestCase):
    def test_create_local_folder_structure_creates_matching_dirs(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue((expected_root / "docs" / "Root Folder" / "Child Folder").exists())
            self.assertFalse((expected_root / "docs" / "Child Folder").exists())

    def test_post_doc_routes_photo_to_year_month_folder(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            storage_root = tmp_path / "storage"
            config_path = tmp_path / "config.json"
